...
Original WildGuard columns are kept unchanged, plus:

| column | meaning |
|---|---|
| `translated_prompt` / `translated_response` | model translation (response = `""` if the original had none) |
...
| `notes` | flags e.g. `no_response`, `empty_translated_response` |
